        ...
        >
          {CERTS.map((cert) => (
            <motion.div
              key={cert.title}
              variants={cardReveal}
              className="glass-premium rounded-2xl p-6 md:p-7 hover:-translate-y-1 transition-all duration-300 group"
            >
              <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${cert.gradient} flex items-center justify-center mb-5 group-hover:scale-105 transition-transform duration-300 shadow-lg`}>
                <cert.icon size={24} className="text-white" />
              </div>
              <h3 className="text-lg font-bold text-white mb-1 group-hover:text-blue-200 transition-colors duration-300">{cert.title}</h3>
              <p className="text-sm text-slate-400 mb-1">{cert.org}</p>
              <span className="text-xs font-mono text-blue-200 bg-blue-500/10 border border-blue-300/15 px-2.5 py-0.5 rounded-full">{cert.year}</span>
